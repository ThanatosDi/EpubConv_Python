            ...
            if format_mode == 'Straight':
                ...
                for File in FileList:
                    ...
                    if not File.find('.css')==-1:
                        with open(f'{os.path.abspath(os.path.join(File,os.path.pardir))}\\Straight.css','w',encoding='utf-8') as FileWrite:
                            FileWrite.write("\nhtml {\n    writing-mode: vertical-rl;\n    -webkit-writing-mode: vertical-rl;\n    -epub-writing-mode: vertical-rl;\n}")
                    if any(File.find(end) for end in ['xhtml', 'html', 'htm']):
                        regex = re.compile(r"<link href=\"../Styles/[\S]*.css\" rel=\"stylesheet\" type=\"text/css\"/>", re.IGNORECASE)
                        fileline = open(File,encoding='utf-8').read()
                        m = re.findall(regex,fileline)
                        modify = re.sub(regex,f'{m}\t\n<link href="../Styles/Straight.css" rel="stylesheet" type="text/css"/>',fileline)
                        open(File,'w',encoding='utf-8').write(modify)
            return True
        except Exception as e:
...
class log:
    ...
    def write(message):
        ...
        # log 資料寫入
        """
        with open( f'epubconv.log','a',encoding='UTF-8') as logfile:
