                ...
                for File in FileList:
                    ...
                    if not File.find('.css')==-1:
                        ...
                        if start is None:
                            ...
                            with open(File,'w+',encoding='utf-8') as css:
                    ...
                    if any(File.find(end) for end in ['xhtml', 'html', 'htm']):
                        ...
                        if not any(re.findall(r'link href=\"../Styles/[\S]*.css\"',lines)):
                            m = re.findall(regex,lines)
                            modify = re.sub(regex,f'\t<link href="../Styles/{(CSSname)}" rel="stylesheet" type="text/css"/>\n</head>',lines)
                            open(File,'w',encoding='utf-8').write(modify)
                    ...
                    if not File.find('.css')==-1:
                        ...
                        if start is not None:
                            if any(re.findall(r'.*writing-mode(?:\:|.*\:).*',lines)):
                                open(File,'w',encoding='utf-8').write(re.sub(r'.*writing-mode(?:\:|.*\:).*','',lines))
                    if not File.find('content.opf')==-1:
